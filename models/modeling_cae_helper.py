...
class VisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, bool_masked_pos):
        ...
        # unmasked embeddings
        x_unmasked = x[~bool_masked_pos].reshape(batch_size, -1, dim) # bool index
...
'''
Encoder that extracts representations with CS Strategies
'''
class ViTEncoder_CS(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=None, init_values=None, attn_head_dim=None,
                 use_abs_pos_emb=True, init_std=0.02, args=None, **kwargs):
    ...
    def forward_features(self, x, bool_masked_pos_a,bool_masked_pos_b):
        x = self.patch_embed(x, bool_masked_pos=bool_masked_pos_a)
        batch_size, seq_len, dim = x.size()

        cls_tokens = self.cls_token.expand(batch_size, -1, -1)

        # unmasked embeddings
        x_unmasked = x[bool_masked_pos_b].reshape(batch_size, -1, dim)
        x_unmasked = torch.cat((cls_tokens, x_unmasked), dim=1)
        ...
        for blk in self.blocks:
            x_unmasked = blk(x_unmasked, bool_masked_pos_a)
    ...
    def forward(self, x, bool_masked_pos_a,bool_masked_pos_b):
        x = self.forward_features(x, bool_masked_pos_a,bool_masked_pos_b)
        return x
...
class ViTEncoder_CSv2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=None, init_values=None, attn_head_dim=None,
                 use_abs_pos_emb=True, init_std=0.02, args=None, **kwargs):
    ...
    def forward_features(self, x, bool_visable_pos):
        x = self.patch_embed(x) #, bool_masked_pos_a
        batch_size, seq_len, dim = x.size()

        cls_tokens = self.cls_token.expand(batch_size, -1, -1)

        # unmasked embeddings
        x_unmasked = x[bool_visable_pos].reshape(batch_size, -1, dim)
        x_unmasked = torch.cat((cls_tokens, x_unmasked), dim=1)

        if self.pos_embed is not None:
            pos_embed = self.pos_embed.expand(batch_size, self.num_patches+1, dim)
            pos_embed_unmasked = pos_embed[:,1:][bool_visable_pos].reshape(batch_size, -1, dim) 
            pos_embed_unmasked = torch.cat((pos_embed[:,:1], pos_embed_unmasked),dim=1)
            x_unmasked = x_unmasked + pos_embed_unmasked
    ...
    def forward(self, x,bool_visable_pos):
        x = self.forward_features(x, bool_visable_pos)
        return x